...
def parse_obj(filename : String, scene : Scene)
  ...
  File.read_lines(filename).each do |line|
    if line.starts_with?("v ")
      values = line.split()
      vertices << Vector3.new(values[1].to_f64, values[2].to_f64, values[3].to_f64)

    elsif line.starts_with?("vn ")
      values = line.split()
      normals << Vector3.new(values[1].to_f64, values[2].to_f64, values[3].to_f64)

    elsif line.starts_with?("f ")
      values = line.split().skip(1).map do |item|
        item.split("/").map do |string|
          string.to_i32
        end
      end
